var prettyTables = {	
	...
	createTable: function(p){
		...
		for (var i = 0; i < p.rows; i++){
			...
			if (i == 0){
				for (var j = 0 ; j < p.columns; j++){
					...
					if (p.links === true && j == p.columns - 1){break;}
					else if (j==0){
		...
		======================================*/ 
		jQuery(p.contain + ' table').tablesorter({sortList:[[1,1],[0,0]]});

		/* SEARCH COMMANDS
		======================================*/    

	   	if (p.search === true){
	   	 	jQuery('#execute-search').on('click', function(){prettyTables.searchTool('#pretty-table-1');});
			jQuery('#pretty-table-search').on('keypress', function(e){
			    var key = e.which;
			    if (key == 13){prettyTables.searchTool('#pretty-table-1');}
			})
	   ...
	   /* META DATA
		...
		for (var j = 0 ; j < p.columns - 1; j++){
			if (j==0){
				jQuery('.pretty-table[data="'+ p.contain +'"] .pretty-table-fixed').append('<div>' + p.rowName + '</div>');
			}
			else {jQuery('.pretty-table[data="'+ p.contain +'"] .pretty-table-fixed').append('<div>' + p.data[0][j]  + '</div>');}
		} 

		/* FIXED HEADER WIDTH
		======================================*/
		var fixedHedWidth = 100 / (p.columns - 1)
...
prettyTables.getAttr('http://bayreporta.com/wp-content/themes/centauri/data/directory.csv', 'pretty-table-1', '1,1', true, '', '', '', 'Game', 'txt', true, true, null);
